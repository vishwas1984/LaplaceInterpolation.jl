...
"""
function punch_holes_3D(centers, radius::Float64, xpoints, ypoints, zpoints)
    clen = length(centers)
    masking_data_points = []
...
"""
function punch_holes_2D(centers, radius::Float64, xpoints, ypoints)
    clen = length(centers)
    masking_data_points = []
    absolute_indices = Int64[]
    for a = 1:clen
